...
for i in range(len(ws)):
    runningtotal = runningtotal + wrange[i]
    wrange[i] = runningtotal
#print "Wsum is " + str(wsum)
#print "Max value is " + str(wrange[N-1])
for i in range(N):
    rnd = random.random() * wsum
    for j in range(len(wrange)):
        if rnd < wrange[j]:
            #print "Match at index " + str(j)
            p3.append(w[j])
            break
#print "Len is " + str(len(p3))
print p3
