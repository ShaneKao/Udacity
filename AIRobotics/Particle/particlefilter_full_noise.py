...
class robot:
    ...
    def sense(self, noise=1): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            ...
            #if noise == 1:
			#	deltay = random.gauss(deltay, self.bearing_noise)
			#	deltax = random.gauss(deltay, self.beraing_noise)
            #print "delta y is " + str(deltay)
            val = atan2(deltay, deltax)
            tval = val - self.orientation
            #newval = tval % ( 2 * pi )
            if noise == 1:
				tval = random.gauss(tval, self.bearing_noise)
			
            Z.append(tval)
            #print "doing bearing " + str(i) + " value is " + str(newval)
...
# motions = [[2. * pi / 10, 20.] for row in range(1)]
