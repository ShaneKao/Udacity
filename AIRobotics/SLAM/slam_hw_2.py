...
def shiftXi(originalXi, newXi):
	for i in range(2,len(originalXi.value)):
		newXi.value[i+2] = originalXi.value[i]
		
	newXi.value[2][0] = 0
	newXi.value[3][0] = 0
	
def shiftOmegas(originalOmega, expandedOmega):
	# move bottom left qudrant down only
	rc = len(originalOmega.value)
	for i in range(0,rc):
		expandedOmega.value[i+2][0] = originalOmega.value[i][0]
		expandedOmega.value[i+2][1] = originalOmega.value[i][1]

	# move top right quadrant over to the right
	for i in range(0, rc):
		expandedOmega.value[0][i+2] = originalOmega.value[0][i]
		expandedOmega.value[1][i+2] = originalOmega.value[1][i]
	
	# thrid quadrant down and right
	for i in range(2, rc):
		for j in range(2,rc):
			expandedOmega.value[i+2][j+2] = originalOmega.value[i][j]
	
	for i in range(rc+2):
		expandedOmega.value[2][i] = 0
		expandedOmega.value[3][i] = 0
		expandedOmega.value[i][2] = 0
		expandedOmega.value[i][3] = 0
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
	...
	poseList = []
	poseList.append([50, 50])

	Omega = matrix()
	rc = (1+num_landmarks)*2
	Omega.zero(rc, rc)
	...
	#for k in range(len(data)):
		...
		thisdata = data[k]
		measurement = thisdata[0]
		movement = thisdata[1]

		for l in range(len(measurement)):
			m = 2 * ( measurement[l][0])

			lIndex = measurement[l][0]

			lmPos = lIndex * 2 + 2
			
			for b in range(2):
				Omega.value[b][b] +=  1.0 / measurement_noise
				Omega.value[lmPos+b][lmPos+b] +=  1.0 / measurement_noise
				
				Omega.value[b][lmPos+b] += -1.0 / measurement_noise
				Omega.value[lmPos+b][b] += -1.0 / measurement_noise
				
				Xi.value[b][0]      += -measurement[l][1+b] / measurement_noise
				Xi.value[lmPos+b][0]      +=  measurement[l][1+b] / measurement_noise
				
				
		# done with the measurements... do the movement
		omegaLen = len(Omega.value)
		
		allRows = []
		for z in range(len(Omega.value)):
			allRows.append(z)
		
		tlist = [0, 1] + range(4, omegaLen+2)

		tOmega = Omega.expand(omegaLen+2, omegaLen+2, tlist, tlist)
		tXi = Xi.expand(omegaLen+2, 1, tlist, [0])


		for b in range(4):
			#print "adding 1 to omega at loc " + str(b) + ", " + str(b)
			tOmega.value[b][b] +=  1.0 / motion_noise
			
		for b in range(2):
			tOmega.value[2+b][b] += -1.0 / motion_noise
			tOmega.value[b][2+b] += -1.0 / motion_noise
			
			tXi.value[b][0]        += -movement[b] / motion_noise
			tXi.value[b+2][0]        +=  movement[b] / motion_noise
		

		aList = []
		for i in range(2,len(tOmega.value)):
			aList.append(i)
			
		OmegaPrime = tOmega.take(aList,aList)
	
	
		A = tOmega.take([0,1],aList)
		B = tOmega.take([0,1])

		
		C = tXi.take([0,1],[0])
		
		xiPrime = tXi.take(aList,[0])
...
## add the +1
#for b in range(4):
#	tmatrix.value[it+b][it+b] += 1
#	
## set the -1 values
#for b in range(1):
#	tmatrix.value[it+b+2][it+b] += -1
#	tmatrix.value[it+b][it+b+2] += -1
#	# xi.value[mindex+b][0] += X
#	# xi.value[mindex+b+2][0] += x
#	
#
#tmatrix.inverse()
#tmatrix.show()
