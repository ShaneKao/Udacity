...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
		...
		# ----------------------------------------
		# compute the CTE
		#cte = calcCTE(myrobot, spath)
		thisPath = spath[index]
		nIndex = index + 1
		nextPoint = spath[nIndex]
		
		dx = nextPoint[0] - thisPath[0]
		dy = nextPoint[1] - thisPath[1]
		
		rx = myrobot.x - thisPath[0]
		ry = myrobot.y - thisPath[1]
		
		robotVec = sqrt(rx*rx + ry*ry)
		pathVec = sqrt(dx*dx+dy*dy)
		#print "pathvec = " + str(pathVec) + " pos vec = " + str(robotVec)
		#print "This point is " + str(thisPath[0]) + ", " + str(thisPath[1])
		#print "next path is " + str(nextPoint[0]) + ", " + str(nextPoint[1])
		#print "Point vec is " + str(sqrt(dx*dx + dy*dy))
		#print "MOve = " + str(N) + " Pos vec is " + str(sqrt(rx*rx + ry*ry)) + " at " + str(myrobot.x) + ", " + str(myrobot.y) + ":" + str(2*pi - myrobot.orientation)
		# dot product
		...
		if not myrobot.check_collision(grid):
			print '##### Collision ####  while at ' + str(myrobot.x) + ", " + str(myrobot.y)
