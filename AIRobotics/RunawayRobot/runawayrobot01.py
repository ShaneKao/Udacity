...
def estimate_next_pos(measurement, OTHER = None):
	...
	if len(OTHER) < 1:
		randomAngle = abs(random.gauss(0, 2*pi))
		heading = randomAngle
		v = random.randint(0,5)
	else:
		dx = measurement[0] - OTHER[len(OTHER) - 1][0][0]
		dy = measurement[1]  - OTHER[len(OTHER) - 1][0][1]

		calcH = atan(dy/dx)
		turning = calcH - OTHER[len(OTHER) - 1][1][1]
		estimatedV = sqrt(dx*dx + dy * dy)
		heading = calcH
		v = estimatedV

		
	calcDX = sin(heading + turning)*v
	
	newY= measurement[1] + sin(heading + turning) * v
	newX = measurement[0] + cos(heading + turning) * v
	xy_estimate = [newX,newY]
	
	OTHER.append( [[measurement[0], measurement[1]], [v, heading, turning]])
