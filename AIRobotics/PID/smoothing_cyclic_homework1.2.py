...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while delta >= tolerance:
		...
		for i in range(len(path)):
			...
			lIndex = (i-1) % len(path)
			nIndex = (i+1) % len(path)
			#print "i is " + str(i) + " next index = " + str(nIndex) + " last index = " + str(lIndex)
			x1 = newpath[i][0] + (weight_data * (path[i][0] - newpath[i][0]))
			x2 = x1 + (weight_smooth * ( newpath[nIndex][0] + newpath[lIndex][0] - 2 * x1 ))
			newpath[i][0] = x2
		
			y1 = newpath[i][1] + weight_data * (path[i][1] - newpath[i][1])
			y2 = y1 + (weight_smooth * ( newpath[nIndex][1] + newpath[lIndex][1] - 2 * y1 ))
			newpath[i][1] = y2
