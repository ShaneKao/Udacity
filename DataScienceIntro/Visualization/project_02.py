...
def plot_weather_data(turnstile_weather):
    ...
    * Which stations have more exits or entries at different times of day
    ...
    #plot = plot + geom_line( colour = "blue", size = 5)
    mydata = turnstile_weather.groupby('Hour', as_index=False).sum()
    mydata.index.name = "Hour"
    plot = ggplot(aes(x='Hour',y='ENTRIESn_hourly'),data=mydata) + geom_line(color="green", size=5) 
    return plot


df = pandas.read_csv("../files/turnstile_data_master_with_weather.csv")
