...
def lineplot(hr_year_csv):
    ...
    #print "file is " + hr_year_csv
    df = pandas.read_csv(hr_year_csv)
    gg = ggplot(aes('yearID','HR'),data=df) + geom_point(color='red') + geom_line(color='red') + ggtitle('Total Homeruns by year') + xlab('Year') + ylab('HR')
    return gg

print lineplot("../files/hr_year.csv")
